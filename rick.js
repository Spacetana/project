...
client.on('message', async message => {
  ...
  if (message.content.startsWith(prefix + "hard")) {
    ...
    let hardlink = ["", "https://cdn.discordapp.com/attachments/731551525691850792/731961158612811847/bifle_biflette_hard_porno_violente_visage_brune.gif", "https://cdn.discordapp.com/attachments/731551525691850792/731965988379033700/29.gif"];

    let hard = new Discord.MessageEmbed()
    .setColor(couleur)
    .setTitle("HARD")
    .setImage(hardlink[Math.floor(Math.random() * (hardlink.length))])
    .setFooter('Rick🛸 ©️ Copyright : Atsuki \\/ Needles', avatarbot)

   if (message.channel.nsfw) return message.channel.send(hard).catch(console.error);
   if (!message.channel.nsfw) return message.channel.send(nonsfw).catch(console.error);
  }  
